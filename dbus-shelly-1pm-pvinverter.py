...
class DbusShelly1pmService:
  def __init__(self, servicename, paths, productname='Shelly(Plus) 1PM', connection='Shelly(Plus) 1PM HTTP JSON service'):
    ...
    self._dbusservice.add_path('/Position', 1) # update for inverter position - 0 - AC IN, 1 - AC Out 1, 2 - AC Out 3
  ...
  def _update(self):   
    try:
       ...
       for phase in ['L1', 'L2', 'L3']:
        ...
        if meter_data['em1:0']['act_power'] < 0:  # Set power to 0 if negative
            power = 0
        else:
            power = meter_data['em1:0']['act_power']
        total = meter_data['em1data:0']['total_act_energy']
        voltage = meter_data['em1:0']['voltage']
        current = meter_data['em1:0']['current']
